...
{
    public function execute(Student $student, array $data, int $userId): bool
    {
        return DB::transaction(function () use ($student, $data, $userId) {
            ...
            if ($student->isDirty()) {
                $changes['students'] = $student->getDirty();
            }
            $student->save();

            $address = $student->address ?? $student->address()->create();
            $address->fill([
            ...
            if ($address->isDirty()) {
                $changes['student_addresses'] = $address->getDirty();
            }
            $address->save();

            $parent = $student->parent ?? $student->parent()->create();
            $parent->fill([
            ...
            if ($parent->isDirty()) {
                $changes['student_parents'] = $parent->getDirty();
            }
            $parent->save();

            $academic = $student->academic ?? $student->academic()->create();
            $academic->fill([
            ...
            if ($academic->isDirty()) {
                $changes['student_academics'] = $academic->getDirty();
            }
            $academic->save();

            $financial = $student->financial ?? $student->financial()->create();
            $financial->fill([
            ...
            if ($financial->isDirty()) {
                $changes['student_financials'] = $financial->getDirty();
            }
            $financial->save();
